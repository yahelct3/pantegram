...
const TicTacToe: NextPage = () => {
  ...
  async function changeBoard(index: number) {
    if (board[index] === "-" && isGameOn) {
      ...
      if (isGameOn) {
        ...
        board = (await axios.get(`http://localhost:8081/nextMove/${strBoard}`)).data.split("");
  ...
  async function endGame() {
    const strBoard = board.join("");
    const winner: string = await (await axios.get(
      `http://localhost:8081/winner/${strBoard}`
    )).data;
  ...
  return (
    <div className="flex justify-center">
      ...
      <button onClick={() => startGame()} className="mt-3.5 bg-base-200 hover:bg-base-300 font-bold py-2 px-4 rounded-full">start new game</button>
